...
That shortcut keeps the same `bash -lic` plus `kimi` bootstrap as the bundled smoke, clears any inherited `OPENAI_BASE_URL` to match the bundled Codex smoke node, and sends one minimal `codex exec` request so raw CLI or provider failures surface before you burn time on a larger smoke run.
