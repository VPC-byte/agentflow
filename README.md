...
`agentflow check-local` prints the Doctor summary first and then launches the bundled smoke pipeline only when the local setup is ready enough to continue. It is the direct CLI equivalent of the repo-local `make check-local` shortcut, and it also accepts an optional custom pipeline path when you want the same doctor-then-run flow for another local Kimi-backed smoke DAG.
...
From the repo root, `make inspect-local`, `make doctor-local`, `make smoke-local`, and `make check-local` wrap the same bundled Kimi-backed workflow with `python3 -m agentflow ...`. If you do not need the Makefile wrapper, `agentflow check-local` is the direct CLI equivalent of `make check-local`.
