...
cleanup() {
  ...
  if [ -f "$stderr_path" ]; then
    printf "\nagentflow smoke stderr:\n" >&2
    sed -n '1,200p' "$stderr_path" >&2
  fi
  if [ -f "$stdout_path" ]; then
    printf "\nagentflow smoke stdout:\n" >&2
...
printf "bundled smoke pipeline path: %s\n" "$bundled_smoke_pipeline"

(
  cd "$repo_root"
  agentflow_run_with_timeout "$python_bin" "$python_bin" -m agentflow smoke "$bundled_smoke_pipeline" --output json-summary --show-preflight >"$stdout_path" 2>"$stderr_path"
)
...
if payload.get("status") != "completed":
    raise SystemExit(f"Unexpected smoke status in stdout JSON: {payload}")
...
print("validated bundled agentflow smoke json-summary stdout and preflight stderr")
PY
